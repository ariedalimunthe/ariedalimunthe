...
const COMMANDS = {
  ...
  social: () => Object.entries(DATA.contact.social)
    ...
    .join('\n'),
  version: () => 'Arie Dalimunthe CLI v1.0.0',
  clear: 'CLEAR',
  gui: 'GUI',
};
...
function formatCommandLine(line: string, currentTheme: string | undefined) {
  ...
  if (line.startsWith('$')) {
    return (
      <>
        <span className={currentTheme === 'dark' ? 'text-fuchsia-500' : 'text-fuchsia-600'}>dev@arie:~</span>
...
export function CliInterface({ onGuiCommand, onMinimize, onMaximize, onClose }: CliInterfaceProps) {
  ...
  useEffect(() => {
    ...
    setIsMounted(true);
    setOutput([
      isMobile ? 'Welcome to Arie Dalimunthe CLI! 👋' : ASCII_ART + '\nWelcome to my portfolio CLI! 👋',
